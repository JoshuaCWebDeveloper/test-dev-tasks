...
`
    ...
    release () {
        const bundleProps = this.__Config.get(['bundleName', 'bundleDir']),
            //set requirements for release
            reqs = {
                remote: this.__Config.get('gitRemote'),
                branch: this.__Config.get('gitSrcBranch'),
                modFiles: [
                    `${bundleProps.bundleDir}/${bundleProps.bundleName}.js`,
                    `${bundleProps.bundleDir}/${bundleProps.bundleName}.min.js`
                ]
            };
        ...
        return Q(Git.Repository.open("./")).then(function (repo) {
            ...
            queries.push(Q.nbind(fs.readFile)(props.changelogName, 'utf8').catch(function (err) {
                //FAILURE
                throw new Error(
`Unable to find file ${props.changelogName}. Create one following changelog specifications. ${err || ''}`
                );
            }));
            ...
            if (statusFail) {
                throw new Error(
`Unrecognized or missing modified files. 
